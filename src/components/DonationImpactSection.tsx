...
const DonationImpactSection = () => {
  const impactAreas = [
    {
      title: "Education",
      impact: "School supplies, uniforms, and learning materials for children",
      icon: <BookOpen className="w-6 h-6" />,
      color: "bg-blue-100 text-blue-600"
    },
    {
      title: "Nutrition",
      impact: "Nutritious meals and feeding programs for growing children",
      icon: <Heart className="w-6 h-6" />,
      color: "bg-red-100 text-red-600"
    },
    {
      title: "Healthcare",
      impact: "Medical care, health screenings, and preventive treatment",
      icon: <Users className="w-6 h-6" />,
      color: "bg-green-100 text-green-600"
    },
    {
      title: "Family Support",
      impact: "Skills training and resources for sustainable family growth",
            ...
            >
              <Card className="h-full bg-white/10 backdrop-blur-md border-white/20 hover:bg-white/20 transition-all duration-300">
                <CardContent className="p-3 md:p-6 text-center">
                  ...
                  <h3 className="text-sm md:text-xl font-bold mb-1 md:mb-3">{area.title}</h3>
                  <p className="text-white/90 text-xs md:text-sm line-clamp-3 md:line-clamp-none">{area.impact}</p>
                </CardContent>
              </Card>
            </motion.div>
          ))}
